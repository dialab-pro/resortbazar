...
class BookingController extends \App\Http\Controllers\Controller
{
    ...
    {
        // dd("dekhi request gula ki ache", $request->all());
        /**
         * @var $booking Booking
         * @var $user User
         */

        //  eikhane validate kortace
        $res = $this->validateDoCheckout();
        if ($res !== true) return $res;
        $user = auth()->user();


        // eikhane booking assign korce
        $booking = $this->bookingInst;
        ...
        if ($booking->pay_now > 0) {
            ...
            // dd($booking->pay_now, get_payment_gateways(), $gatewayObj, !empty($rules['payment_gateway']), empty($gateways[$payment_gateway]), !class_exists($gateways[$payment_gateway]), !$gatewayObj->isAvailable());
