...
$menus = [
    ...
    'boat-booking-history' => [
        'url'      => route("user.boat_booking_history"),
        'title'    => __("Boat Booking History"),
        'icon'     => 'fa fa-clock-o',
        'position' => 23
    ],
    "wishlist"=>[
...
{{-- this option modify  --}}
    <div class="user-profile-plan">
        ...
        @if(!Auth::user()->hasPermission("dashboard_agent_access") and setting_item('agent_enable'))
            <a href=" {{ route("user.upgrade_agent") }}">{{ __("Become a Agent") }}</a>
        @endif

    </div>
 {{-- need to be modify for customer profile --}}
