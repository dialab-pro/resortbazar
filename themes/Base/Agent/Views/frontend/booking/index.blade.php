        ...
        <div class="tabbable">
            <ul class="nav nav-tabs ht-nav-tabs">
                ...
                <li class="@if(empty($status_type)) active @endif">
                    <a href="{{route("agent.booking.index")}}">{{__("All Booking")}}</a>
                </li>
                @if(!empty($statues))
                    @foreach($statues as $status)
                        <li class="@if(!empty($status_type) && $status_type == $status) active @endif">
                            <a href="{{route("agent.booking.index",['status'=>$status])}}">{{booking_status_to_text($status)}}</a>
                        </li>
                    @endforeach
                @endif
            </ul>
                        ...
                        <table class="table table-bordered table-striped table-booking-history">
                            ...
                            <tbody>
                                @foreach($bookings as $booking)
                                    @include('Agent::frontend.'.ucfirst($booking->object_model).'.bookingHistory.loop')
                                @endforeach
                            </tbody>
