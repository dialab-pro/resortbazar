...
@endphp
<div class="destination-item @if(!$row->image_id) no-image  @endif" style="{{ $destination_style }}">

    @if(!empty($link_location)) <a href="{{$link_location}}">  @endif
        <div class="image" @if($row->image_id) style="background: url({{$row->getImageUrl()}});{{ $image_style }}" @endif >
