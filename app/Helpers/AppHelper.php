...
function generate_timezone_list()
    ...
    function getBookedNotify(){
        ...
        $notify = $checkNotify->where('read_at', null)->get();
        $allServices = get_bookable_services();

        $bookedNotify = [];
        foreach ($notify as $oneNotification){
            $oneNotificationData = json_decode($oneNotification['data'], true);
            $notification = $oneNotificationData['notification'];
            if (isset($notification['type']) && array_key_exists($notification['type'], $allServices)) {
                $bookedNotify[] = [
                    'notification' => $notification,
                    'type_model' => $allServices[$notification['type']],
                ];
            }
        }


        $notify = count($bookedNotify);
        // Log::channel('result')->info($notify);
...
function select2($name,$options,$old = [],$multiple = false,$attrs = []){
    ?>
